...
i64 res[101][1011];
bool vis[101][1011];
i64 ways(i64 id, i64 taka)
{
    // base case or known coin
    if(taka == 0) return 1;
    if(taka < 0) return 0;
    if(id == n) return 0;
    if(vis[id][taka] == 1) return res[id][taka];
    vis[id][taka] = 1;

    for(int i = 0; i <= coin[id]; i++){
        res[id][taka]= (res[id][taka] % M + ways(id + 1, taka - (i*type[id])) % M) % M;
    }

    return res[id][taka] % M;
}
int main ()
{
