    ...
    while(test--){
        ...
        while(q--){
            ...
            if(ck==0){
                ...
                scanf("%lld%lld", &x, &v);
                if(x!=0){
                    // so do positive summation after that negative summation above equation
                    sum+=(x*a[x]);
                    sum-=(n-x-1)*a[x];
                    a[x]=v; // changing the value
                    sum-=(x*a[x]); // doing the above formula
                    sum+=(n-x-1)*a[x];
                }
                else{
                    // do negative for only zero
                    sum-=(n-1)*a[0];
                    a[0]=v;
                    sum+=(n-1)*a[0];
